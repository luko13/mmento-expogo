...
export const signIn = async (email: string, password: string): Promise<boolean> => {
  try {
    ...
    // Intentar login
    const { user, session } = await authService.signIn(email.trim().toLowerCase(), password);
...
/**
 * Verificar y configurar cifrado para usuarios existentes
 */
export const ensureEncryptionSetup = async (password: string): Promise<boolean> => {
  try {
    ...
    const hasEncryption = await authService.hasEncryptionSetup(user.id);
    if (hasEncryption) {
      console.log("✅ Cifrado ya configurado");
      return true;
    }

    console.log("🔧 Configurando cifrado...");
    const success = await authService.setupEncryptionForExistingUser(user.id, password);
    
    if (success) {
      console.log("✅ Cifrado configurado exitosamente");
    } else {
      console.warn("⚠️ No se pudo configurar el cifrado");
    }
...
export const debugAuth = async (): Promise<void> => {
  ...
  try {
    ...
    console.log("3️⃣ Usuario actual:", user ? `✅ ${user.email}` : "❌ No autenticado");
    ...
    // 5. Test crypto service
    const { CryptoService } = await import("./cryptoService");
    const cryptoService = CryptoService.getInstance();
    const cryptoOk = await cryptoService.testCryptoService();
    console.log("5️⃣ Crypto service:", cryptoOk ? "✅ OK" : "❌ Error");
