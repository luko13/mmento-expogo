...
export class FileEncryptionService {
  ...
  private async encryptLargeFileOptimized(
    ...
    console.log(`📦 Procesando ${totalChunks} chunks de ${(CHUNK_SIZE / 1024).toFixed(0)}KB con ${PARALLEL_CHUNKS} en paralelo`);
    ...
    for (let i = 0; i < totalChunks; i += PARALLEL_CHUNKS) {
      ...
      for (let j = 0; j < PARALLEL_CHUNKS && i + j < totalChunks; j++) {
        ...
        batch.push({
          index: chunkIndex,
          promise: this.processStreamChunk(fileUri, offset, length, { key: symmetricKey, nonce, counter: 0 }, chunkIndex)
        });
      }
      ...
      console.log(`📊 Progreso: ${(progress * 100).toFixed(0)}% - Batch en ${Date.now() - batchStart}ms`);
    }
  ...
  private async uploadEncryptedFile(
    ...
    mimeType: string,
    fileSize: number,
    encryptedKeys: Array<{ userId: string; encryptedKey: string; nonce: string }>,
    authorUserId: string
  ...
  async smartCompress(uri: string, mimeType: string): Promise<string> {
    try {
      ...
      if (mimeType.includes('image') && fileSize > 2 * 1024 * 1024) {
        ...
        console.log(`✅ Image compressed: ${(fileSize / 1024 / 1024).toFixed(2)}MB → ~${(fileSize * 0.3 / 1024 / 1024).toFixed(2)}MB`);
  ...
  async encryptAndUploadFile(
    ...
    try {
      ...
      console.log(`📁 Procesando archivo: ${fileName} (${(fileSize / 1024 / 1024).toFixed(2)}MB)`);
      ...
      console.log(`✅ Archivo cifrado y subido en ${(totalTime / 1000).toFixed(2)}s`);
  ...
  async batchEncryptAndUploadFilesOptimized(
    ...
    const { smallPhotos, smallVideos, mediumFiles, largeFiles } = this.groupFilesBySizeAndType(fileAnalysis);
    ...
    if (smallPhotos.length >= MIN_BATCH_SIZE) {
      console.log(`📦 Combining ${smallPhotos.length} small photos into archive`);
      ...
      try {
        ...
        console.error('Archive creation failed, falling back to individual uploads', error);
    ...
    if (mediumFiles.length > 0) {
      console.log(`🚀 Processing ${mediumFiles.length} medium files in parallel`);
    ...
    if (largeFiles.length > 0) {
      console.log(`📡 Processing ${largeFiles.length} large files with streaming`);
      ...
      for (const file of largeFiles) {
        ...
        console.log(`✅ ${file.fileName} processed in ${(duration / 1000).toFixed(2)}s`);
  ...
  private async encryptLargeFile(
    ...
    console.log(`📦 Procesando ${totalChunks} chunks de ${(this.CHUNK_SIZE / 1024).toFixed(0)}KB`);
    ...
    for (let i = 0; i < totalChunks; i += PARALLEL_CHUNKS) {
      ...
      for (let j = 0; j < PARALLEL_CHUNKS && i + j < totalChunks; j++) {
        ...
        batch.push({
          index: chunkIndex,
          promise: this.processStreamChunk(fileUri, offset, length, streamState, chunkIndex)
        });
      }
  ...
  private async processStreamChunk(
    ...
    try {
      ...
      // Encrypt chunk
      const { encrypted } = await this.cryptoService.encryptFile(chunkBuffer, streamState.key);
      return encrypted;
    } catch (error) {
   ...
   */
  private async readFileChunk(fileUri: string, offset: number, length: number): Promise<string> {
    try {
   ...
   */
  private deriveChunkNonce(baseNonce: Uint8Array, chunkIndex: number): Uint8Array {
  ...
  private async encryptKeysForRecipients(
    ...
    return results.filter(Boolean) as Array<{ userId: string; encryptedKey: string; nonce: string }>;
  }
  ...
  private async saveMetadataAndKeys(
    ...
    // Save keys
    const keysPromises = metadata.encryptedKeys.map(keyData =>
      supabase
    ...
    const [metadataResult, ...keysResults] = await Promise.all([
      metadataPromise,
      ...keysPromises
    ]);
    ...
    if (metadataResult.error) {
      ...
      throw new Error(`Error guardando metadata: ${metadataResult.error.message}`);
    }

    // Log key errors but don't fail
    keysResults.forEach((result, index) => {
      if (result.error) {
        console.error(`Error guardando clave ${index}:`, result.error);
      }
    });
  }
   ...
   */
  private async analyzeFiles(files: Array<{ uri: string; fileName: string; mimeType: string }>) {
    return Promise.all(
   ...
   */
  private groupFilesBySizeAndType(files: Array<{ uri: string; fileName: string; mimeType: string; size: number }>) {
  ...
  private async processFilesInParallel(
    files: Array<{ uri: string; fileName: string; mimeType: string; size: number }>,
  ...
  private async createAndEncryptArchive(
    ...
    const symmetricKey = await this.cryptoService.generateSymmetricKey();
    const { encrypted, nonce } = await this.cryptoService.encryptFile(archiveData, symmetricKey);
  ...
  /**
   * Downloads and decrypts a file with optimizations
   */
  async downloadAndDecryptFile(
    ...
    try {
      ...
      // 1. Get metadata and key in parallel
      console.time('→ obtener metadata');
      const [metadataResult, keyResult] = await Promise.all([
        supabase
          .from('encrypted_files')
          .select('*')
          .eq('file_id', fileId)
      ...
      console.timeEnd('→ obtener metadata');
      ...
      const fileMetadata = metadataResult.data;
      const keyData = keyResult.data;

      // 2. Get author's public key and decrypt symmetric key
      console.time('→ descifrar clave');
      ...
      console.timeEnd('→ descifrar clave');
      ...
      if (downloadError || !encryptedFile) {
        throw new Error(`Error descargando archivo: ${downloadError?.message || 'Unknown error'}`);
      }

      // 4. Convert to Uint8Array efficiently
      console.time('→ conversión');
      const fileBuffer = await this.blobToUint8Array(encryptedFile);
      console.timeEnd('→ conversión');

      // 5. Decrypt file
      console.time('→ descifrado');
      const fileNonce = await workerManager.base64ToBuffer(fileMetadata.file_nonce);
      ...
      console.timeEnd('→ descifrado');

      const totalTime = Date.now() - startTime;
      console.log(`✅ Archivo descargado y descifrado en ${(totalTime / 1000).toFixed(2)}s`);
