...
import {
  type EncryptedTechnique,
  type TechniqueDBRecord,
} from "../../types/encryptedTechnique";
...
export default function AddTechniqueWizardEncrypted({
  ...
  const steps = [
    {
      title: t("basicInformation", "Información Básica"),
      component: TechniqueBasicsStepEncrypted,
    },
    {
      title: t("details", "Detalles"),
      component: TechniqueDetailsStepEncrypted,
    },
  ];
  ...
  const encryptAllSensitiveFields = async (
    data: EncryptedTechnique
  ): Promise<EncryptedTechnique> => {
    if (!keyPair) {
    ...
    try {
      ...
      if (data.description?.trim()) {
        encryptedFields.description = await encryptForSelf(
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!keyPair) {
        Alert.alert(
          ...
          t(
            ...
            "Configura el cifrado antes de guardar la técnica"
          ),
          [
      ...
      const encryptedTechniqueData = await encryptAllSensitiveFields(
        techniqueData
      );
      ...
      if (
        ...
        techniqueData.image_url.startsWith("file://")
      ) {
      ...
      if (
        ...
        techniqueData.video_url.startsWith("file://")
      ) {
        const metadata = await fileEncryptionService.encryptAndUploadFile(
          techniqueData.video_url,
    ...
    if (createdItemId) {
      try {
        ...
        if (techniqueData) {
          ...
          if (techniqueData.technique_categories?.[0]?.category_id) {
            const { data: categoryData } = await supabase
              .from("user_categories")
              .select("name")
              .eq("id", techniqueData.technique_categories[0].category_id)
              .single();
          ...
          const navigationData = {
            ...
            effect: techniqueData.description || "",
            secret: techniqueData.notes || "",
            effect_video_url: techniqueData.video_url,
            secret_video_url: null,
            photo_url: techniqueData.image_url,
            photos: [],
            script: "",
            angles: techniqueData.angles || [],
            duration: 0,
            reset: 0,
            difficulty: techniqueData.difficulty || 0,
            is_encrypted: techniqueData.is_encrypted,
            notes: techniqueData.notes,
          };

          // Si creaste /technique/[id].tsx, usa esta ruta:
          router.push({
            pathname: "/technique/[id]",
            params: {
              id: createdItemId,
              technique: JSON.stringify(navigationData),
            },
          });
        }
      } catch (error) {
  ...
  return (
    <>
      ...
      <EncryptionSetup
        ...
        onSetupComplete={() => {}}
      />
      {/* Modal de éxito al crear técnica */}
      <SuccessCreationModal
        visible={showSuccessModal}
        onClose={handleCloseSuccessModal}
        onViewItem={handleViewItem}
        onAddAnother={handleAddAnother}
        itemName={techniqueData.name || t("common.technique", "Technique")}
        itemType="technique"
      />
    </>
  );
