...
export default function Tricks() {
  ...
  const fetchTricks = async () => {
    ...
    if (user) {
      const { data, error } = await supabase
        .from("magic_tricks")
        .select(
          "id, title, effect, difficulty, created_at, duration, is_public, status"
        )
        .eq("user_id", user.id)
  ...
  const filterTricks = () => {
    ...
    if (selectedDifficulty) {
      filtered = filtered.filter(
        (trick) => trick.difficulty === selectedDifficulty
      );
    }
  ...
  const renderTrickItem = ({ item }: { item: Trick }) => (
    <StyledTouchableOpacity className="bg-white/10 p-4 rounded-lg mb-3">
  ...
  const renderFilterOptions = () => (
    <StyledView className="mb-4">
      ...
      <StyledView className="flex-row flex-wrap mb-2">
        {["beginner", "easy", "intermediate", "advanced", "expert"].map(
          (difficulty) => (
            <StyledTouchableOpacity
              key={difficulty}
              onPress={() =>
      ...
      <StyledView className="flex-row flex-wrap">
        {["draft", "published", "archived"].map((status) => (
          <StyledTouchableOpacity
            ...
            className={`m-1 px-3 py-2 rounded-full ${
              selectedStatus === status ? "bg-emerald-600" : "bg-white/20"
            }`}
          >
            <StyledText className="text-white">
              {t(`status.${status}`)}
